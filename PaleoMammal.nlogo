...
to go
  ...
  ask turtles [move-herbivores]
  ask lions [move-lions]

  ask turtles [set hours-since-water hours-since-water + 1]
...
to move-herbivores
  ...
  if is-SNSBr? self [
    ...
    ifelse hours-since-water > hours-without-water-SNSBr[
      let target min-one-of water-patches [distance myself]
      ;let target one-of water-patches
      ifelse distance target < 1
      [ move-to target ]
      [ fd 1 ]

      if [vt] of patch-here = 1 [ set hours-since-water 0]
    ]
    [
...
to load-animals
  ...
  ask turtles [set hours-since-water random 4]
